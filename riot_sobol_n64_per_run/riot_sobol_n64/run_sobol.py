...
def run_one(job: tuple[int, tuple[float, ...], str]) -> tuple:
    sample_id, values, run_dir_str = job
    ...
    try:
        ...
        np.save(run_dir / f"run_{sample_id:04d}.npy", measurement, allow_pickle=False)
    except Exception as exc:
...
def main():
    ...
    run_dir.mkdir(parents=True, exist_ok=True)
    jobs = [(i, tuple(map(float, row)), str(run_dir)) for i, row in enumerate(samples)]
    ...
    metadata = {
        ...
        "spatial_warmup": {
            "fine_entropy_cv_threshold": 0.05,
            "window": 10,
        ...
        "per_run_files": {
            "directory": "runs",
            "pattern": "run_XXXX.npy",
            "records_per_file": MEASUREMENT_STEPS,
            "dtype_fields": list(MEASUREMENT_DTYPE.names),
        },
    }
