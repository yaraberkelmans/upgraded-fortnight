...
class MixedFootballRiotsParams:
    ...
    schelling_weight: float = 2.00
    count_empty_as_different: bool = True

    # Civil Violence / fan-fan game parameters.
    aggression_threshold: float = 0.10

    # Baseline aggression is now an agent attribute, not a HOME-AWAY relation.
    # It is assigned per group from the global composition:
    # HOME fan baseline = 1 - home_fraction; AWAY fan baseline = home_fraction.
    base_aggression_weight: float = 1.00
    hostility_weight: float = 0.50
    rival_weight: float = 1.00

    # Positive same-group effect: more own fans nearby increases confidence /
    # group pressure, and therefore raises the chance of fighting.
    own_group_weight: float = 0.70

    # Local contagion: already-aggressive neighbours increase aggression.
...
class MixedFootballRiotsModel(Model):
    ...
    def __init__(self, params: MixedFootballRiotsParams | None = None, **kwargs):
        ...
        self.datacollector = DataCollector(
            model_reporters={
                ...
                "Average base aggression": lambda m: m.average_base_aggression(),
                ...
                "Unhappy fans": lambda m: m.count_unhappy_fans(),
    ...
    def group_base_aggression(self, group: FanGroup) -> float:
        """Return the baseline aggression assigned to each fan of a group.

        For now this is not an individual random draw and not a relationship
        matrix. Every fan in the same group gets the same baseline value. The
        baseline is linked to group size: the smaller group has the higher
        baseline aggression.

        Example with home_fraction=0.70:
        - HOME baseline = 1 - 0.70 = 0.30
        - AWAY baseline = 1 - 0.30 = 0.70
        """
        if group == FanGroup.HOME:
            return 1.0 - self.params.home_fraction
        return self.params.home_fraction
    ...
    def count_unhappy_fans(self) -> int:
        return sum(not fan.happy for fan in self.fans)

    def average_base_aggression(self) -> float:
        if not self.fans:
            return 0.0
        return sum(fan.base_aggression for fan in self.fans) / len(self.fans)
